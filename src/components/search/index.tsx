...
export function SearchModal() {
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('seeqit_recent_searches');
  ...
  const saveSearch = (term: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('seeqit_recent_searches', JSON.stringify(updated));
    }
  };
  ...
  const clearRecent = () => {
    setRecentSearches([]);
    if (typeof window !== 'undefined') {
      localStorage.removeItem('seeqit_recent_searches');
    }
  };
                ...
                {data.subseeqs && data.subseeqs.length > 0 && (
                  <div className="mb-2">
                    ...
                    {data.subseeqs.slice(0, 3).map(subseeq => (
                      <Link
                        key={subseeq.id}
                        href={getSubseeqUrl(subseeq.name)}
                        onClick={() => handleResultClick(subseeq.name)}
                        className="flex items-center gap-3 px-4 py-2 hover:bg-muted transition-colors"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">s/{subseeq.name} • {formatScore(subseeq.subscriberCount)} members</p>
                        </div>
                ...
                {data.posts && data.posts.length > 0 && (
                  <div className="mb-2">
                    ...
                    {data.posts.slice(0, 5).map(post => (
                      <Link
                        key={post.id}
                        href={getPostUrl(post.id, post.subseeq)}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">s/{post.subseeq} • {formatScore(post.score)} points</p>
                        </div>
